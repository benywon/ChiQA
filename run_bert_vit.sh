...
deepspeed --include='localhost:0,1,2,3,4,5,6,7' --master_addr 'localhost' --master_port 10015 ${base_dir}/train_bert_vit.py \
    --deepspeed \
...
ps -ef|grep train_bert_vit|awk '{print $2}'|xargs kill -9
sleep 10

data_name=test
test_data_dir=${base_dir}/data
echo $test_data_dir
model_name=bert_vit_base_models
echo $model_name
...
do
    ...
    CUDA_VISIBLE_DEVICES=$cuda_idx python3 -u test_bert_vit.py \
